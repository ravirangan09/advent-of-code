...
const points = [[current_col, current_row]];
...
let step = 1;
while (!loop_complete) {
  points.push([current_col, current_row]);
...
points.push(points[0]); //keep first point as last point for closed loop

let count = 0;
for (let i = 0; i < lines.length; i++) {
  for (j = 0; j < lines[0].length; j++) {
    count += inside([j, i], [points]) ? 1 : 0;
  }
}
